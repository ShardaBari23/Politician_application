...
function openTab(evt, tabId) {
  ...
  if (btn) btn.style.display = tabId === "voters" ? "inline-block" : "none";
}
...
function fetchSearchVoters() {
  ...
  fetch(`/api/voters/search?q=${searchQuery}&limit=${limit}&offset=${offset}`, {
    headers: { Authorization: "Bearer " + token }
  })
    .then(res => res.json())
    .then(data => {
...
function uploadExcel() {
  ...
  if (!file) {
    msg.innerText = "Select Excel file";
    msg.style.color = "red";
    return;
  }
    ...
    .then(data => {
      new Chart(surnameChart, {
        ...
        data: {
          ...
          datasets: [{ label: "Total", data: data.map(d => d.count) }]
        }
      });
